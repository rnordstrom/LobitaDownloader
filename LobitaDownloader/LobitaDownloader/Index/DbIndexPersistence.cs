...
namespace LobitaDownloader
{
    public class DbIndexPersistence : IIndexPersistence
    {
        ...
        public void CleanTagLinks(List<string> tagNames)
        {
            try
            {
                string idCmd;
                string linksIdCommand;
                string deleteCmd;
                int id = 0;
                List<int> linkIds;
                StringBuilder sb;
                ...
                {
                    idCmd = $"SELECT id FROM tags WHERE name = '{t}'";

                    MySqlCommand cmd = new MySqlCommand(idCmd, conn);
                    ...
                    linksIdCommand = $"SELECT link_id FROM tag_links WHERE tag_id = {id}";
                    cmd = new MySqlCommand(linksIdCommand, conn);
                    ...
                    deleteCmd = $"DELETE FROM tags WHERE id = '{id}'";
                    cmd = new MySqlCommand(deleteCmd, conn);

                    cmd.ExecuteNonQuery();

                    deleteCmd = $"DELETE FROM links WHERE id IN ({sb})";
                    cmd = new MySqlCommand(deleteCmd, conn);
